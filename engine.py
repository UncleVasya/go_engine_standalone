...
def get_bots (bots, indices):
    result = []
    for index in indices:
        result.append(bots[index])
    return result
...
def get_moves(game, bots, bot_nums, bots_to_play, time_limit, timebank, fischer_time, turn):
    bot_finished = [not game.is_alive(bot_nums[b]) for b in range(len(bots))]
    bot_moves = [[] for b in bots]
    error_lines = [[] for b in bots]
    statuses = [None for b in bots]
    time_used = [0 for b in bots]
...
#        time.sleep(0.01) # Why is this here?
        for b, bot in enumerate(bots):
            if bot_nums[b] not in bots_to_play:
            ...
            if not bot.is_alive:
                ...
                while line != None:
                    ...
                    line = bot.read_error()
                bot_finished[b] = True
                time_used[b] = time.time() - start_time
            ...
            for x in range(100):
                ...
                if game.bot_input_finished(line):
...
#            if line.lower() == 'go':
    ...
    # kill timed out bots
    #print((bot_finished, bot_nums, bots_to_play, time_limit))
    ...
    return bot_moves, error_lines, statuses, time_used
